...
#
def setup_config_for_testbed(duthosts, rand_one_dut_hostname, nbrhosts, ptfhost, test_vm_names, filepath):
    ...
    for vm in test_vm_names:
        ...
        vmhost.command("sudo cp /etc/sonic/config_db.json /etc/sonic/config_db.json.back")
        vmhost.command("sudo config reload /tmp/{}.json -y".format(vm))


    for vm in test_vm_names:
        vmhost = nbrhosts[vm]['host']
        pytest_assert(wait_until(1200, 20, 0, check_interface_status_of_up_ports, vmhost),
