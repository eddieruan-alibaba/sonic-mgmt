...
def check_topo_recv_pkt_raw(ptfadapter, port=0, dst_ip="", dscp = 0, no_packet = False, no_vlan=True, validateDSCP = True):
    ...
    # mask.set_do_not_care_packet(scapy.Ether, 'dst')
    # mask.set_do_not_care_packet(scapy.Ether, 'src')
    # mask.set_do_not_care_packet(scapy.Dot1Q, 'vlan')
    # mask.set_do_not_care_packet(scapy.IP, "ihl")
    # mask.set_do_not_care_packet(scapy.IP, "tos")
    # mask.set_do_not_care_packet(scapy.IP, "len")
    # mask.set_do_not_care_packet(scapy.IP, "id")
    # mask.set_do_not_care_packet(scapy.IP, "flags")
    # mask.set_do_not_care_packet(scapy.IP, "frag")

    # mask.set_do_not_care_packet(scapy.IP, "ttl")
    # mask.set_do_not_care_packet(scapy.IP, "src")
    # mask.set_do_not_care_packet(scapy.IP, "chksum")
    # mask.set_do_not_care_packet(scapy.UDP, "chksum")
    # mask.set_do_not_care_packet(scapy.UDP, "len")
    ## mask.set_do_not_care_packet(scapy.IP, "dst")
    if "." in dst_ip:
        if validateDSCP:
