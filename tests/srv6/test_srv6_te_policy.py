...
def test_te_policy_func_dt46(nbrhosts):
    ...
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'ipv6 route 1::1/128 blackhole vrf Vrf1'")
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'segment-routing' -c 'srv6' -c 'locators' -c "
                                           + "'locator lsid1 prefix fd00:201:201::/48 block-len 32 node-len 16 func-bits 32 ' -c 'no opcode ::fff1:1:0:0:0'")
    ret = check_static_route_func(nbrhosts["PE1"]['host'], 'fd00:201:201:fff1:1::/80', 'End.DT46')
    pytest_assert(ret == False, "Del static route fail")
    ret = check_bgp_route_is_exist(nbrhosts["PE3"]['host'], 'show bgp ipv4 vpn 192.100.0.1')
    pytest_assert(ret == False, "Check 192.100.0.1 is exist")
    ret = check_bgp_route_is_exist(nbrhosts["PE3"]['host'], 'show bgp ipv6 vpn 1::1')
    pytest_assert(ret == False, "Check 1::1 is exist")
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'segment-routing' -c 'srv6' -c 'locators' -c "
                                           + "'locator lsid1 prefix fd00:201:201::/48 block-len 32 node-len 16 func-bits 32 ' -c 'opcode ::fff1:1:0:0:0 end-dt46 vrf Vrf1' ")
    ret = check_static_route_func(nbrhosts["PE1"]['host'], 'fd00:201:201:fff1:1::/80', 'End.DT46')
    pytest_assert(ret == True, "No static route")
    ret = check_vpn_route_is_te(nbrhosts["PE3"]['host'], ['192.100.0.1'])
    pytest_assert(ret == True, "Check 192.100.0.1 is not te")
    ret = check_vpn_route_is_te(nbrhosts["PE3"]['host'], ['1::1'], True)
    pytest_assert(ret == True, "Check 1::1 is not te")
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'no ipv6 route 1::1/128 blackhole vrf Vrf1'")
    logger.info("End test_te_policy_func_dt46")

def test_te_policy_func_dt4_dt6(nbrhosts):
    ret = check_vpn_route_is_te(nbrhosts["PE3"]['host'], ['192.100.0.1'],"Vrf1")
    pytest_assert(ret == True, "Check 192.100.0.1 is not te")
    ret = check_static_route_func(nbrhosts["PE1"]['host'], 'fd00:201:202:fff1:1::/80', 'End.DT4')
    pytest_assert(ret == True, "Static check route fail")
    ret = check_static_route_func(nbrhosts["PE1"]['host'], 'fd00:201:202:fff1:2::/80', 'End.DT6')
    pytest_assert(ret == True, "Static check route fail")
    ret = check_vpn_route_sid_func(nbrhosts["PE3"]['host'], '192.100.0.1', 'fd00:201:201:fff1:1::')
    pytest_assert(ret == True, "Check remote sid fail")
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'router bgp 64600 vrf Vrf1' -c 'no srv6-locator lsid1'")
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'router bgp 64600 vrf Vrf1' -c 'srv6-locator lsid2'")
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'ipv6 route 1::1/128 blackhole vrf Vrf1'")
    ret = check_vpn_route_sid_func(nbrhosts["PE3"]['host'], '192.100.0.1', 'fd00:201:202:fff1:1::')
    pytest_assert(ret == True, "Check remote sid fd00:201:202:fff1:1:: fail")
    ret = check_vpn_route_sid_func(nbrhosts["PE3"]['host'], '1::1', 'fd00:201:202:fff1:2::', True)
    pytest_assert(ret == True, "Check remote sid fd00:201:202:fff1:2:: fail")
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'router bgp 64600 vrf Vrf1' -c 'no srv6-locator lsid2'")
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'router bgp 64600 vrf Vrf1' -c 'srv6-locator lsid1'")
    time.sleep(10)
    ret = check_vpn_route_sid_func(nbrhosts["PE3"]['host'], '192.100.0.1', 'fd00:201:201:fff1:1::')
    pytest_assert(ret == True, "Check remote sid fd00:201:201:fff1:1:: fail")
    ret = check_vpn_route_sid_func(nbrhosts["PE3"]['host'], '1::1', 'fd00:201:201:fff1:1::', True)
    pytest_assert(ret == True, "Check remote sid fd00:201:201:fff1:1:: fail")
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'no ipv6 route 1::1/128 blackhole vrf Vrf1'")
    logger.info("End test_te_policy_func_dt4_dt6")

def test_te_policy_func_end_endx(nbrhosts):
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'segment-routing' -c 'srv6' -c 'locators' -c "
                                           + "'locator lsid1 prefix fd00:201:201::/48 block-len 32 node-len 16 func-bits 32 ' -c 'opcode ::fff1:2:0:0:0 end' ")
    ret = check_static_route_func(nbrhosts["PE1"]['host'], 'fd00:201:201:fff1:2::/80', 'End')
    pytest_assert(ret == True, "No static fd00:201:201:fff1:2::/80 route")
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'segment-routing' -c 'srv6' -c 'locators' -c "
                                           + "'locator lsid1 prefix fd00:201:201::/48 block-len 32 node-len 16 func-bits 32 ' -c 'opcode ::fff1:3:0:0:0 end-x interface Ethernet2 nexthop 120.2.0.2' ")
    ...
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'segment-routing' -c 'srv6' -c 'locators' -c "
                                           + "'locator lsid1 prefix fd00:201:201::/48 block-len 32 node-len 16 func-bits 32 ' -c 'opcode ::fff1:4:0:0:0 end-x interface Ethernet2 nexthop fc02::2' ")
    ...
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'segment-routing' -c 'srv6' -c 'locators' -c "
                                           + "'locator lsid1 prefix fd00:201:201::/48 block-len 32 node-len 16 func-bits 32 ' -c 'no opcode ::fff1:2:0:0:0' ")
    ret = check_static_route_func(nbrhosts["PE1"]['host'], 'fd00:201:201:fff1:2::/80', 'End')
    pytest_assert(ret == False, "No static fd00:201:201:fff1:2::/80 route")
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'segment-routing' -c 'srv6' -c 'locators' -c "
                                           + "'locator lsid1 prefix fd00:201:201::/48 block-len 32 node-len 16 func-bits 32 ' -c 'no opcode ::fff1:3:0:0:0' ")
    ret = check_static_route_func(nbrhosts["PE1"]['host'], 'fd00:201:201:fff1:3::/80', 'End.X')
    pytest_assert(ret == False, "No static fd00:201:201:fff1:3::/80 route")
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'segment-routing' -c 'srv6' -c 'locators' -c "
                                           + "'locator lsid1 prefix fd00:201:201::/48 block-len 32 node-len 16 func-bits 32 ' -c 'no opcode ::fff1:4:0:0:0' ")
    ret = check_static_route_func(nbrhosts["PE1"]['host'], 'fd00:201:201:fff1:4::/80', 'End.X')
    pytest_assert(ret == False, "No static fd00:201:201:fff1:4::/80 route")
    logger.info("End test_te_policy_func_end_endx")

def test_te_policy_func_locator(nbrhosts):
    ret = check_vpn_route_is_te(nbrhosts["PE3"]['host'], ['192.100.0.1'])
    pytest_assert(ret == True, "Check 192.100.0.1 is not te")
    nbrhosts["PE1"]['host'].command("sudo vtysh -c 'configure terminal' -c 'router bgp 64600 vrf Vrf1' -c 'no srv6-locator lsid1'")
    ret = check_bgp_route_is_exist(nbrhosts["PE3"]['host'], 'show bgp ipv4 vpn 192.100.0.1')
    ...
    time.sleep(10)
    ret = check_bgp_route_is_exist(nbrhosts["PE3"]['host'], 'show bgp ipv4 vpn 192.100.0.1')
    pytest_assert(ret == True, "Check 192.100.0.1 is not exit")
    ret = check_vpn_route_is_te(nbrhosts["PE3"]['host'], ['192.100.0.1'],"Vrf1")
    pytest_assert(ret == True, "Check 192.100.0.1 is not te")
    logger.info("End test_te_policy_func_locator")
...
def test_traffic_multi_policy_check_4(tbinfo, duthosts, rand_one_dut_hostname, ptfhost, nbrhosts, ptfadapter):
    ...
    nbrhosts['P2']['host'].shell("sudo vtysh -c  'configure terminal' -c 'router bgp 65102' -c 'address-family ipv6 unicast' -c ' redistribute static route-map srv6_r'")
    pytest_assert(wait_until(100, 1, 0, check_bfd_status, nbrhosts['PE3']['host'], ["b"], ["c"]),
              "Bfd not established!")
